          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                {/* Total Saldo */}
                <div className="bg-gradient-to-br from-blue-600 to-blue-700 p-5 rounded-2xl text-white shadow-lg shadow-blue-100">
                  <div className="flex justify-between items-center opacity-90 mb-2">
                    <span className="text-xs font-medium uppercase tracking-wider">Total Saldo Gabungan</span>
                    <Wallet className="w-5 h-5" />
                  </div>
                  <h3 className="text-2xl font-extrabold">{formatIDR(totalWalletBalance)}</h3>
                  <p className="text-xs opacity-75 mt-2">Dari {wallets.length} rekening keluarga</p>
                </div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <h3 className="text-2xl font-bold text-slate-900">{formatIDR(totalIncome)}</h3>
                  <p className="text-xs text-emerald-600 font-semibold mt-2">Arus kas masuk</p>
                </div>

                {/* Total Pengeluaran */}
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="flex justify-between items-center text-slate-400 mb-2">
                    <span className="text-xs font-medium uppercase tracking-wider">Pengeluaran Bulan Ini</span>
                    <TrendingDown className="w-5 h-5 text-rose-500" />
                  </div>
                  <h3 className="text-2xl font-bold text-slate-900">{formatIDR(totalExpense)}</h3>
                  <div className="mt-2 flex items-center space-x-1">
                    <span className="text-xs text-slate-500">Mencapai </span>
                    <span className="text-xs font-bold text-blue-600">
                      {totalBudget > 0 ? Math.round((totalExpense / totalBudget) * 100) : 0}%
                    </span>
                    <span className="text-xs text-slate-500">dari target budget</span>
                  </div>
                </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                <h4 className="font-bold text-slate-900 text-base mb-4">Pemantauan Anggaran Bulanan</h4>
                      ...
                      <div key={cat.id} className="space-y-1.5">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-xs text-slate-500">
                            {formatIDR(spent)} / <strong className="text-slate-700">{formatIDR(cat.budget)}</strong>
                          </span>
                        </div>
                ...
                >
                  Lihat Seluruh Anggaran <ChevronRight className="w-3.5 h-3.5 ml-0.5" />
                </button>
              </div>

              {/* TRANSAKSI TERAKHIR & DISTRIBUSI PENGGUNA */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                {/* 5 Transaksi Terakhir */}
                <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="flex justify-between items-center mb-4">
                    <h4 className="font-bold text-slate-900 text-base">Aktivitas Terakhir</h4>
                    <button 
                      onClick={() => {
                  ...
                  <div className="divide-y divide-slate-100 max-h-80 overflow-y-auto pr-1">
                    {transactions.slice(0, 5).map(tx => {
                      ...
                      return (
                        <div key={tx.id} className="py-3 flex justify-between items-center">
                          <div className="flex items-center space-x-3">
                            <span className={`w-2.5 h-2.5 rounded-full ${cat?.color || 'bg-blue-500'}`} />
                            <div>
                              <p className="text-sm font-semibold text-slate-800 line-clamp-1">{tx.description}</p>
                              <div className="flex items-center space-x-2 text-xs text-slate-500 mt-0.5">
                                <span className="font-medium">{member?.name || 'Keluarga'}</span>
                                <span>•</span>
                                <span>{tx.date}</span>
                              </div>
                            </div>
                    ...
                    {transactions.length === 0 && (
                      <p className="text-center py-6 text-slate-400 text-sm">Belum ada transaksi bulan ini.</p>
                    )}
                  </div>
                </div>

                {/* Pembelanja Terbesar (Anggota) */}
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                  <div>
                    <h4 className="font-bold text-slate-900 text-base mb-3">Distribusi Anggota</h4>
                    <p className="text-xs text-slate-500 mb-4">Total pengeluaran yang dilakukan oleh masing-masing anggota keluarga.</p>
                    
                    <div className="space-y-4">
                      {members.map(m => {
                        const spent = expenseByMember[m.id] || 0;
                        const percentage = totalExpense > 0 ? Math.round((spent / totalExpense) * 100) : 0;
                        return (
                          <div key={m.id} className="flex items-center justify-between">
                            <div className="flex items-center space-x-2.5">
                              <span className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white ${m.avatarColor}`}>
                                {m.name.charAt(0)}
                              </span>
                              <div>
                            ...
                            <div className="text-right">
                              <p className="text-xs font-bold text-slate-900">{formatIDR(spent)}</p>
                              <span className="text-[10px] text-slate-500 font-semibold">{percentage}%</span>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <button 
                    ...
                    className="mt-6 w-full py-2 bg-slate-50 text-slate-600 hover:bg-slate-100 rounded-xl text-xs font-bold transition-all text-center"
                  >
                    Atur Anggota Keluarga
                  </button>
                </div>
